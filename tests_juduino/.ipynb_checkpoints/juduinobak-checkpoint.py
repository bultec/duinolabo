...
class Interface:
    ...
    '''
    def __init__(self, p_port='', p_grandeurs=[], p_mode='temporel'): # temporel ou points
        '''
        initialise la classe
        entrée : p_port -> str : port série auquel est connecté arduino (voir la documentation de pyduino)
                 p_grandeurs -> list : tableau de tuple contenat les grandeurs à mesurer et leurs unités
        ...
        self.zone_affichage = widgets.Output(layout=widgets.Layout(margin='20px 0 0 20px'))
    ...
    def mesures_tempo(self, chaine, n, affichage=None, fs=[]):
        ...
        '''
        if affichage == None:
            affichage = affichage = self.update
        return self.arduino.mesures_tempo(chaine, n, affichage, fonctions=fs)
