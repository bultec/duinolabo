...
void mesures(int command) {
  int uc = 0;
  int E = 0;
  unsigned long dt = t_total / (nb_points - 1); //en ms
...
void loop() {
  if (Serial.available()) {
    ...
    if (chaine == "charge") mesures(1);
    else if (chaine == "decharge") mesures(0);
    else if ((chaine.substring(0,3) == "cnt") || (chaine.substring(0,3) == "dnt")){
      // on décompose la chaine     
      // la chaine est de la forme cnt_50_7000 ou dnt_50_7000
      int i = chaine.indexOf("_",0);
      int j = chaine.indexOf("_",i+1);
      nb_points = chaine.substring(i+1,j).toInt();
      t_total = chaine.substring(j+1).toInt();
      if (chaine.substring(0,3) == "cnt") mesures(1);
      else mesures(0);
    }                                            
